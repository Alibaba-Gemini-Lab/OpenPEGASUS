...
using default_safe_ptr =
    safe_ptr<T, std::recursive_mutex, std::unique_lock<std::recursive_mutex>, std::unique_lock<std::recursive_mutex>>;

#ifdef SHARED_MTX  // C++14
template <typename T>
using shared_mutex_safe_ptr = safe_ptr<T,
...
protected:
    ...
    template <typename req_lock>
    using auto_lock_t = typename safe_ptr<T, mutex_t, x_lock_t, s_lock_t>::template auto_lock_t<req_lock>;
    template <typename req_lock>
    using auto_lock_obj_t = typename safe_ptr<T, mutex_t, x_lock_t, s_lock_t>::template auto_lock_obj_t<req_lock>;
...
public:
    ...
    template <typename req_lock>
    using auto_lock_t = typename safe_obj<T, mutex_t, x_lock_t, s_lock_t>::template auto_lock_t<req_lock>;
    template <typename req_lock>
    using auto_lock_obj_t = typename safe_obj<T, mutex_t, x_lock_t, s_lock_t>::template auto_lock_obj_t<req_lock>;
...
class lock_timed_any {
    ...
    std::unique_lock<mtx_t> try_lock_one(mtx_t& mtx) const {
        ...
        while (!lock.try_lock()) {
            auto const time_remained = duration(deadlock_timeout) - std::chrono::duration_cast<duration>(
...
public:
    ...
    bool try_lock() {
        if (!lock_flag.test_and_set(std::memory_order_acquire)) {
            ...
            if (owner_thread_id.load(std::memory_order_acquire) != get_fast_this_thread_id()) return false;
        }
        ++recursive_counter;
        return true;
    }
...
class contention_free_shared_mutex {
    ...
    // struct cont_free_flag_t { alignas(std::hardware_destructive_interference_size) std::atomic<int> value;
    ...
    int get_or_set_index(index_op_t index_op = get_index_op, int set_index = -1) {
        ...
        if (it != thread_local_index_hashmap.cend()) set_index = it->second.thread_index;
...
public:
    ...
    int register_thread() {
        ...
        if (cur_index == -1) {
            if (shared_locks_array_ptr.use_count() <= (int)shared_locks_array.size())  // try once to register thread
            {
                ...
                // std::cout << "\n thread_id = " << std::this_thread::get_id() << ", register_thread_index = " <<
                // cur_index <<
    ...
    void lock_shared() {
        ...
        if (register_index >= 0) {
            ...
            if (owner_thread_id.load(std::memory_order_acquire) != get_fast_this_thread_id()) {
                size_t i = 0;
                for (bool flag = false; !want_x_lock.compare_exchange_weak(flag, true, std::memory_order_seq_cst);
                     flag      = false)
                    if (++i % 100000 == 0) std::this_thread::yield();
                owner_thread_id.store(get_fast_this_thread_id(), std::memory_order_release);
            }
    ...
    void lock() {
        ...
        int const register_index = get_or_set_index();
        if (register_index >= 0) assert(shared_locks_array[register_index].value.load(std::memory_order_acquire) == 1);
...
template <typename T>
using contfree_safe_ptr = safe_ptr<T,
                                   contention_free_shared_mutex<>,
...
public:
    ...
    void get_range_lower_upper(const key_t& low, const key_t& up, result_vector_t& result_vec) const {
        ...
        auto end_it            = (const_part.upper_bound(up) == const_part.cend()) ? const_part.cend()
    ...
    void erase_lower_upper(const key_t& low, const key_t& up) {
        auto end_it = (partition->upper_bound(up) == partition->end()) ? partition->end()
    ...
    void clear() {
        for (auto it = partition->begin(); it != partition->end(); ++it) it->second->clear();
    }
};
// ---------------------------------------------------------------
}  // namespace sf
