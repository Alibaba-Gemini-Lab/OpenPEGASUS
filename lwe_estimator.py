...
def estimate_lwe(n, alpha=None, q=None, secret_distribution=True, m=oo, # noqa
    ...
    EXAMPLE::
        ...
        usvp: rop:  ≈2^57.2,  red:  ≈2^57.2,  δ_0: 1.009214,  β:  101,  d:  349,  m:      220
         dec: rop:  ≈2^61.5,  m:      229,  red:  ≈2^61.5,  δ_0: 1.009595,  β:   93,  d:  357,  ...
        dual: rop:  ≈2^81.4,  m:      376,  red:  ≈2^81.4,  δ_0: 1.008810,  β:  111,  d:  376,  ...

        sage: d = estimate_lwe(**Param.LindnerPeikert(256, dict=True))
        usvp: rop: ≈2^137.0,  red: ≈2^137.0,  δ_0: 1.005788,  β:  228,  d:  588,  m:      331
...
def estimate_Pegasus(n, q, stddev):
    alpha = alphaf(sigmaf(stddev), q);
    secret_dist = ((-1, 1, 0), 64) # ternay secret with a hamming weight
    m = oo # the attacker can use as many samples he wishes 
    print("n = {}, logq = {}, sigma = {}".format(n, math.log(q, 2), stddev))
    print("CLASSICAL PRIMAL")
    print(primal_usvp(n, alpha, q, secret_distribution=secret_dist, m=m, success_probability=0.99, reduction_cost_model=BKZ.sieve))
    print("CLASSICAL DUAL")
    print(dual_scale(n, alpha, q, secret_distribution=secret_dist, m=m, success_probability=0.99, reduction_cost_model=BKZ.sieve))
    reduction_cost_model =  lambda beta, d, B: ZZ(2)**RR(0.292*beta)
    print("CLASSICAL PRIMAL (conservative)")
    print(primal_usvp(n, alpha, q, secret_distribution=secret_dist, m=m, success_probability=0.99, reduction_cost_model=reduction_cost_model))
    print("CLASSICAL DUAL (conservative)")
    print(dual_scale(n, alpha, q, secret_distribution=secret_dist, m=m, success_probability=0.99, reduction_cost_model=reduction_cost_model))

def estimate_Chimera(n, q, stddev):
    alpha = sqrt(2*pi)*stddev
    secret_dist = (1, 0) # binary secret 
    m = oo # the attacker can use as many samples he wishes 
    print("n = {}, logq = {}, sigma = {}".format(n, math.log(q, 2), stddev))
    print("CLASSICAL PRIMAL")
    print(primal_usvp(n, alpha, q, secret_distribution=secret_dist, m=m, success_probability=0.99, reduction_cost_model=BKZ.sieve))
...
"""
n = 1024, logq = 45.0, sigma = 8192.0
CLASSICAL PRIMAL
rop: 2^150.3, red: 2^150.3, delta_0: 1.003921, beta:  409, d: 2788, m: 1763
CLASSICAL DUAL
rop: 2^159.7, m: 1907, red: 2^159.7, delta_0: 1.003724, beta:  441, repeat: 2^107.4, d: 2931, c: 1
CLASSICAL PRIMAL (conservative)
rop: 2^119.4, red: 2^119.4, delta_0: 1.003921, beta:  409, d: 2788, m: 1763
CLASSICAL DUAL (conservative)
rop: 2^131.2, m: 1927, red: 2^131.2, delta_0: 1.003674, beta:  449, repeat: 2^81.0, d: 2951, c: 1
"""
estimate_Pegasus(1024, 2.**45, 2.**13)

"""
n = 4096, logq = 105.0, sigma = 1024.0
CLASSICAL PRIMAL
rop: 2^163.9, red: 2^163.9, delta_0: 1.003668, beta:  450, d: 8864, m: 2^12.2
CLASSICAL DUAL
rop: 2^169.8, m: 2^12.3, red: 2^169.8, delta_0: 1.003560, beta:  470, repeat: 2^53.2, d: 9159, c: 1
CLASSICAL PRIMAL (conservative)
rop: 2^131.4, red: 2^131.4, delta_0: 1.003668, beta:  450, d: 8864, m: 2^12.2
CLASSICAL DUAL (conservative)
rop: 2^137.2, m: 2^12.3, red: 2^137.2, delta_0: 1.003560, beta:  470, repeat: 2^53.2, d: 9159, c: 1
"""
estimate_Pegasus(4096, 2.**105, 2.**10)

"""
CLASSICAL PRIMAL
rop: 2^143.6, red: 2^143.6, delta_0: 1.004067, beta:  388, d: 1958, m: 933
CLASSICAL DUAL
rop: 2^153.9, m: 1019, red: 2^153.9, delta_0: 1.003832, beta:  423, repeat: 2^95.0, d: 2043, c: 724.077
CLASSICAL PRIMAL (conservative)
rop: 2^113.3, red: 2^113.3, delta_0: 1.004067, beta:  388, d: 1958, m: 933
CLASSICAL DUAL (conservative)
rop: 2^125.9, m: 1033, red: 2^125.9, delta_0: 1.003782, beta:  431, repeat: 2^78.0, d: 2057, c: 724.077
"""
estimate_Chimera(1024, 2**32, 2**(-23))

"""
CLASSICAL PRIMAL
rop: 2^177.8, red: 2^177.8, delta_0: 1.003414, beta:  498, d: 7978, m: 2^11.9
CLASSICAL DUAL
rop: 2^182.2, m: 2^12.0, red: 2^182.2, delta_0: 1.003346, beta:  513, repeat: 2^101.4, d: 8181, c: 2^113.3
CLASSICAL PRIMAL (conservative)
rop: 2^145.4, red: 2^145.4, delta_0: 1.003414, beta:  498, d: 7978, m: 2^11.9
CLASSICAL DUAL (conservative)
rop: 2^153.0, m: 2^12.0, red: 2^153.0, delta_0: 1.003296, beta:  524, repeat: 2^46.5, d: 8243, c: 2^113.3
"""
estimate_Chimera(4096, 2**196, 2**(-50))
